...
async function requestUserCode(): Promise<{ userCode: string; deviceCode: string; verificationUri: string }> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to request user code (${res.status}): ${text}`)
  }
...
async function pollForToken(deviceCode: string): Promise<OAuthToken> {
  ...
  throw new Error("Timed out waiting for Qwen authentication")
}

export async function startQwenOAuth(authStore: AuthStore): Promise<OAuthToken> {
  ...
  console.log("Qwen Authentication")

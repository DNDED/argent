...
export async function startXaiOAuth(authStore: AuthStore): Promise<OAuthToken> {
  ...
  console.log("Opening browser for xAI Grok authentication...")
  ...
  const token = await exchangeCodeForToken(code, callbackPort, verifier, discovery.token_endpoint)
  authStore.setToken("xai", token)

  console.log("xAI authentication successful!")
  return token
}
